defmodule Mix.Tasks.AvenUi.Add do
  ...
  @impl Mix.Task
  def run(args) do
    {opts, components, _} = OptionParser.parse(args,
    ...
    if components == [] do
      Mix.shell().error("No components specified.\n\nUsage:\n  mix aven_ui.add button badge\n  mix aven_ui.add --all")
      exit({:shutdown, 1})
    end
    ...
    Enum.each(components, fn name ->
      copy_file(dep_path(Map.fetch!(@component_files, name)),
                Path.join(dest_dir, "#{name}.ex"),
                dry_run, :green, &rewrite_namespace/1)
    end)

    copy_file(dep_path("lib/aven_ui/helpers.ex"),
              Path.join(dest_dir, "helpers.ex"),
              dry_run, :green, &rewrite_namespace/1)

    copy_file(dep_path("assets/css/avenui.css"),
              Path.join(File.cwd!(), "assets/css/avenui.css"),
              dry_run, :blue, & &1)

    copy_file(dep_path("assets/js/hooks/index.js"),
  ...
  defp copy_file(source, dest, dry_run, color, transform) do
    if File.exists?(dest) do
      ...
      Mix.shell().info([color, if(dry_run, do: "  dry-run", else: "  create"), :reset, "  #{dest}"])
      unless dry_run do
  ...
  defp dep_path(relative) do
    case :code.priv_dir(:aven_ui) do
      {:error, _} -> Path.join([__DIR__, "../../../../..", relative]) |> Path.expand()
      priv        -> Path.join([Path.dirname(priv), relative]) |> Path.expand()
    end
  end
  ...
  defp print_next_steps(components, dest_dir, false) do
    ...
    imports = Enum.map_join(components, "\n      ", fn c ->
