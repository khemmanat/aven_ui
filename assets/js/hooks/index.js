...
const Dropdown = {
  mounted() {
    ...
    this.items   = () => this.menu?.querySelectorAll("[data-avn-dropdown-item]") ?? [];
    this.open    = false;
    ...
    document.addEventListener("click",         this.outsideClick = () => this.close());
    document.addEventListener("keydown",       this.escClose = (e) => e.key === "Escape" && this.close());
  },
...
const Modal = {
  mounted() {
    ...
    // Trap focus
    this.el.addEventListener("keydown", this.trapFocus = (e) => {
      if (e.key !== "Tab") return;
    ...
    // Backdrop click
    this.el.addEventListener("click", this.backdropClose = (e) => {
...
const Tooltip = {
  mounted() {
    ...
    this.hide = () => { this.tip.style.opacity = "0"; };
    ...
    this.observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting && !this.loading) {
          this.loading = true;
          this.pushEvent("load_more", {}, () => { this.loading = false; });
        }
      });
    }, { rootMargin: "200px" });
...
export { Flash, Dropdown, Modal, Tooltip, AutoResize, CopyToClipboard, InfiniteScroll, ScrollTop };
